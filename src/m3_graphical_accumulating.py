...
def draw_parallel_lines(n, point, length, window):
    ...
    for _ in range(n):
        line = rg.Line(rg.Point(x_coordinate, y_coordinate + total),rg.Point(x_coordinate + length, y_coordinate + total))
        total = total + 30
...
def draw_lines(n, point, window):
    ...
    for k in range(n):
        apex = (200 / n) * k + (point.y - 100)
        line = rg.Line(point, rg.Point(point.x + 100, apex))
        line.attach_to(window)
